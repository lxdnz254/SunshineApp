...
public class SettingsActivity extends PreferenceActivity
    ...
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
        if ( key.equals(getString(R.string.pref_location_key)) ) {
            ...
            // Remove attributions from any PlacePicker locations
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == PLACE_PICKER_REQUEST){
            ...
            if (resultCode == RESULT_OK){
                ...
                // If the provided place does not have an address, we'll form a display-friendly
                ...
                // Tell the SyncAdapter that we've changed the location,so that we can update our
                // UI with new values. We need to do this manually because we are responding to the
                // PlacePicker widget result here instead of allowing the LocationEditTextPreference
                // to handle these changes and invoke our callbacks.
                Preference locationPreference = findPreference(getString(R.string.pref_location_key));
                setPreferenceSummary(locationPreference, address);

                // Add attributions to our new PlacePicker location
                if (mAttribution != null) {
